...
async function main(){
  ...
  for(const [name,color,description] of [['agent-review','9d6a18','Coding-agent result is ready for human PR review'],['mission-complete','19765d','Approved mission completed through a verified PR'],['runner-blocked','b64a4a','Mission runner needs intervention']])await ensureLabel(name,color,description);
  ...
  for(const source of (sourceIssues||[]).filter(x=>!x.pull_request)){
    const comments=await api(sourceToken,`/repos/${repoPath(sourceRepo)}/issues/${source.number}/comments?per_page=100`),target=targetFromComments(comments);if(!target)continue;
    const targetIssue=await api(runnerToken,`/repos/${repoPath(target.repo)}/issues/${target.number}`);const prNumber=await findPull(target);
    if(prNumber){
      const pr=await api(runnerToken,`/repos/${repoPath(target.repo)}/pulls/${prNumber}`);
      if(pr.merged_at){await addLabels(source.number,['mission-complete']);for(const l of ['agent-running','agent-review','runner-blocked','runner-dispatched'])await removeLabel(source.number,l);await commentOnce(source.number,'<!-- runner-complete -->',`🎉 **Mission completed.** The coding-agent PR was verified and merged: ${pr.html_url}. Mission Control may now celebrate this win and recompute the next move.`);await api(sourceToken,`/repos/${repoPath(sourceRepo)}/issues/${source.number}`,{method:'PATCH',body:{state:'closed',state_reason:'completed'}});continue}
      if(pr.state==='open'){await addLabels(source.number,['agent-review']);await removeLabel(source.number,'agent-running');await commentOnce(source.number,`<!-- runner-review:${target.repo}#${pr.number} -->`,`✅ **Result ready for review.** The agent opened ${pr.html_url}. No merge/deploy is performed by Mission Control.`);continue}
    }
    if(targetIssue.state==='closed'){await addLabels(source.number,['runner-blocked']);await removeLabel(source.number,'agent-running');await commentOnce(source.number,'<!-- runner-closed-without-pr -->',`⚠️ The target runner issue closed without a merged PR: ${targetIssue.html_url}. Human inspection is required.`)}else{await addLabels(source.number,['agent-running']);await removeLabel(source.number,'agent-review')}
  }
