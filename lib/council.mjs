...
function demoReply(agent, topic, spokenIds) {
  const fragments = {
    chief: `Company operating review for “${topic}”: I’d separate what needs founder judgement from what the team can resolve itself. The highest-leverage move is to turn the current focus into one owned milestone, expose the blocker, and close the loop before creating more work.`,
    ...
    operator: `For “${topic}”, I’d force the work into owner → dependency → next action → definition of done. Anything without an owner or completion condition is not yet an operating plan.`,
    growth: `For “${topic}”, I’d choose one real user or partner segment and one distribution experiment that teaches us whether the product deserves to spread.`
  };
  let text = fragments[agent.id] || `My contribution to “${topic}” is to turn my speciality into one concrete decision for the room.`;
  if (agent.id === 'chief' && !spokenIds.has('builder')) text += ` [[SUMMON:builder]] [[TASK:Verify the highest-leverage company milestone|builder|high]]`;
...
function chooseDemoAgents(agents, text) {
  ...
  if (/company|founder|priority|operate|strategy|attention|focus/.test(lower)) add('chief');
