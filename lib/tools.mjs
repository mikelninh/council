  ...
  for (const run of runs) {
    ...
    if (run.tool === 'github.portfolio.scan') {
      ...
      const rows = (run.data?.repos || []).map((r, i) => {
        const desc = String(r.description || '').replace(/\s+/g, ' ').slice(0, 180) || '(no description)';
        const topics = r.topics?.length ? ` topics=${r.topics.join(',')}` : '';
        const readme = r.readme ? ` | README: ${String(r.readme).replace(/\s+/g, ' ').slice(0, 900)}` : '';
        return `${i + 1}. ${r.fullName} | ${r.visibility} | ${r.language} | stars=${r.stars} forks=${r.forks} issues=${r.openIssues} sizeKB=${r.size} | pushed=${r.pushedAt || 'unknown'} | ${desc}${topics}${readme}`;
      });
      blocks.push([
        `TOOL github.portfolio.scan: SUCCESS`,
        `Scope: ${s.scope}; user=${s.username}; repositories=${s.count}; private=${s.privateCount}; public=${s.publicCount}; active30d=${s.active30}; active90d=${s.active90}; README inspected=${s.readmeCount}/${s.inspectableCount} (${s.readmeCoverage}%); totalStars=${s.stars}.`,
