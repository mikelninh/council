...
async function readJson(req) { let body=''; for await (const chunk of req) { body += chunk; if (body.length > 1_500_000) throw Object.assign(new Error('Request too large.'), { status:413 }); } return body ? JSON.parse(body) : {}; }
...
server.listen(port, () => { const s=status(); console.log(`Council v3 running at http://localhost:${port} (${s.live ? s.model : 'demo mode'})`); });
